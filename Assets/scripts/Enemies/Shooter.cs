...
public class Shooter : MonoBehaviour, IEnemy
{
    ...
    [SerializeField] private int projectliesPerBurst;
    ...
    private IEnumerator ShootRoutine()
    {
        ...
        {
            for (int j = 0; j < projectliesPerBurst; j++)
            {
    ...
    private void TargetConeOfInfluence(out float startAngle, out float currentAngle, out float angleStep)
    {
        ...
        if (angleSpread != 0)
        {
            angleStep = angleSpread / (projectliesPerBurst - 1);
            halfAngleSpread = angleStep / 2f;
